...
#include "UnityStandardCausticsInput.cginc"
...
)
{
	...
	color += UNITY_BRDF_GI (s.diffColor, s.specColor, s.oneMinusReflectivity, s.oneMinusRoughness, s.normalWorld, -s.eyeVec, occlusion, gi);
		...
		if (s.posWorld.y < _CausticsStartLevel) {
			...
			float2 causticsTextureCoord = mul(worldCoord, _CausticsLightOrientation).xy;
			// Calculate caustics light emission
			color += Emission(causticsTextureCoord) * fadeFactor * belowFactor;
		}
	#endif
